...
export function float01(
  ...
  if (value < min || value > max) {
    simpleDebugWarring(`value ${value} is out of range [${min}, ${max}]`);
  }
  if (reset < min || reset > max) {
    (`reset ${reset} is out of range [${min}, ${max}]`);
  }
...
function FindProgram(name: string): ProgramBuilder {
  ...
  if (!node) simpleDebugWarring(`Program ${name} not found`);
...
function resolveProgramOrder(): string[] {
  ...
  function visit(name: string, path: Set<string>) {
    if (path.has(name)) {
      simpleDebugWarring(`Cycle detected in program dependencies: ${[...path, name].join(" -> ")}`);
    }
...
function resolveTriggerOrder(): string[] {
  ...
  function visit(name: string, path: Set<string>) {
    if (path.has(name)) simpleDebugWarring(`循环依赖: ${[...path, name].join(" -> ")}`);
